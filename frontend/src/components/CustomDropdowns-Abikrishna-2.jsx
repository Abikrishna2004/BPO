...
// --- Animation Variants ---
// --- Animation Variants ---
const dropdownVariants = (placement = 'bottom', inline = false) => ({
    hidden: {
        opacity: 0,
        y: inline ? 0 : (placement === 'bottom' ? -10 : 10),
        height: inline ? 0 : 'auto',
        scale: inline ? 1 : 0.95,
        filter: "blur(10px)",
...
export function CustomSelect({ options, value, onChange, placeholder, icon: Icon, centered = false, placement = 'bottom' }) {
    ...
    return (
        <div ref={containerRef} className="relative w-full z-50">
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        variants={dropdownVariants(placement)}
                        initial="hidden"
                        animate="visible"
                        exit="exit"
                        className={`absolute ${placement === 'top' ? 'bottom-[calc(100%+8px)] origin-bottom' : 'top-[calc(100%+8px)] origin-top'} left-0 w-full bg-[#09090b]/95 border border-white/10 rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] overflow-hidden z-[9999] ring-1 ring-white/5 backdrop-blur-3xl`}
                    >
...
export function CustomRoleSelect({ categories, value, onChange, placeholder, centered = true, placement = 'bottom', inline = false }) {
    const [isOpen, setIsOpen] = useState(false);
    const containerRef = useRef(null);
    ...
    return (
        <div ref={containerRef} className={`relative w-full ${inline ? 'z-0' : 'z-40'}`}>
            <button
                type="button"
                onClick={() => setIsOpen(!isOpen)}
                className={`w-full bg-white/5 border ${isOpen ? 'border-primary ring-2 ring-primary/50 shadow-[0_0_30px_rgba(59,130,246,0.3)] bg-black/80' : 'border-white/10 hover:border-white/30 hover:bg-white/10'} rounded-xl py-4 px-5 text-sm text-left flex items-center justify-between transition-all duration-300 group backdrop-blur-xl relative overflow-hidden`}
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        variants={dropdownVariants(placement, inline)}
                        initial="hidden"
                        animate="visible"
                        exit="exit"
                        className={`${inline ? 'relative mt-2' : 'absolute ' + (placement === 'top' ? 'bottom-[calc(100%+8px)] origin-bottom' : 'top-[calc(100%+8px)] origin-top')} left-0 w-full bg-[#09090b]/98 border border-white/10 rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.8)] overflow-hidden z-[9999] max-h-[220px] overflow-y-auto custom-scrollbar ring-1 ring-white/10 backdrop-blur-3xl`}
                    >
