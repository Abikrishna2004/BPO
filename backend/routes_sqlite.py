...
def calculate_efficiency(db: Session, user_id: int) -> float:
    # Base Score - Increased starting point
    base_score = 60.0
    ...
    att_score = 0
    if total_days > 0:
        att_score = (present_days / total_days) * 30
    
    # Attendance Streak Bonus
    # Calculate current streak of consecutive present days
    streak = 0
    today = datetime.now().date()
    # Get last 30 days attendance sorted new to old
    recent_att = db.query(models.Attendance).filter(models.Attendance.user_id == user_id).order_by(models.Attendance.date.desc()).limit(30).all()
    ...
    streak_bonus = min(streak * 2, 20) # Max 20 points for 10 day streak

    # Penalty: Reduced impact
    att_penalty = absent_days * 2

    # 2. Task Completion (Weight: 40)
    ...
    task_score = 0
    if total_tasks > 0:
        task_score = (c_count / total_tasks) * 40
    
    # Task Volume Bonus (Reward for doing MORE work, not just ratio)
    volume_bonus = c_count * 0.5 # 0.5 points per completed task
    ...
    for t in c_tasks:
        if t.deadline and t.completed_at:
            if t.completed_at < t.deadline:
                ...
                # Late: -2 flat per late task (Reduced from 5)
                late_penalty += 2
    ...
    total_eff = (base_score + att_score + streak_bonus + task_score + volume_bonus + 
                 speed_bonus + ach_score + call_score - att_penalty - late_penalty)
